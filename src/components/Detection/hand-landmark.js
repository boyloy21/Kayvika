		...
		if (
			...
			if (video.currentTime > 0) {
				...
				if (result.landmarks && result.handedness) {
					...
					// handsData.forEach(({ landmark }) => detectSign(landmark));
					// const hands = handsData.map(({ landmark }) => landmark);
					// console.log(hands.handedness);
				}
			}
		}
	...
	return (
		<section>
			<div className='relative w-full pt-[56.25%]'>
				...
				{/* <div className='absolute top-4 left-4 bg-white p-4 rounded-md shadow-md z-20'>
					<h2 className='text-xl font-bold'>Sign Detected:</h2>
					<p className='text-lg'>{signDetected}</p>
				</div> */}
				<Webcam
					width='1280'
					height='720'
